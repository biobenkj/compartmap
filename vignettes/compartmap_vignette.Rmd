...
```{r postprocessInflections, message=FALSE}
...
The currently recommended input files for scRNA-seq is single-cell bigWigs, though does work with a counts based object as demonstrated below. Single-cell bigWigs can be generated through several tools, such as deeptools (https://deeptools.readthedocs.io/en/latest/). To import bigWigs, we can use the `importBigWig` function in compartmap. This will read in a bigWig file and optionally summarize to an arbitrary bin size. The bin size used in the compartmap manuscript was 1kb and is the default for the function.
